...
// funkcija za filtriranje istitravanja tipkala
bool filtered_pin_state(uint8_t _port, uint8_t _pin, bool _value, uint16_t T);

// funkcije za provjeru padaju?eg i rastu?eg brida na digitalnom ulazu
bool isFalling_edge(uint8_t _port, uint8_t _pin);
bool isRising_edge(uint8_t _port, uint8_t _pin);

// kreiranje adrese za EEPROM memoriju
#define addr_byte(x) (uint8_t *)(x)
#define addr_word(x) (uint16_t *)(x)
#define addr_dword(x) (uint32_t *)(x)
#define addr_float(x) (float *)(x)
